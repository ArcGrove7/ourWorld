...
| [`account.js`](account.js) | 帳號名正規化：去掉紀錄檔封存時加上的 `-YYYYMMDD-HHMM[-說明]` 後綴。**同一顆帳號在資料裡有好幾個名字**是沒有徵兆的錯——去重去不掉、一顆帳號的歷史被切成好幾段 | 無 |
| [`derived.js`](derived.js) | 衍生表的兩道閘門：`shrinkGate()` 擋用不齊的紀錄重跑把證據洗掉，`stamp()`／`verify()` 擋來源改了衍生表沒重跑 | 無 |

相依只有一條線：`http` → `budget`、`token`。其他九支彼此無關，單獨拿一支走也可以。
